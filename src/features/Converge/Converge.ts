...
export type ConvergeResult = Convergence & {
  /** The machine as it was read last, which is the reading the proof was judged against. */
  snapshot: FactSnapshot;
};

/**
 * `converge` — take a machine from what it is to what its blueprint declares.
 *
...
export class Converge {
  async execute(request: ConvergeRequest): Promise<ConvergeResult> {
    ...
    for (let pass = 1; ; pass += 1) {
      ...
      const done = (end: ConvergenceEnd): ConvergeResult =>

...
| `qemu additional arguments` - свойство скалярного вида, принимается scripting | пробрасывает порт напрямую аргументами QEMU, но дублирует то, что UTM должен делать сам |

Проверка `qemu additional arguments` не проводилась - это следующий шаг перед тем, как описывать интерфейс провайдера.
